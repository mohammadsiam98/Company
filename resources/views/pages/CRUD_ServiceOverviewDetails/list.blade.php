                    ...
                    <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                        <div class="widget-content widget-content-area br-6">
                            <table class="multi-table table table-hover" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($ServiceOverviewDetails as $key =>$ServiceOverview)
                                    <tr>
                                        <td>{{$key +1 }}</td>
                                        <td>{{$ServiceOverview->page_heading}}</td>
                                        <td>{{$ServiceOverview->service_overview_title}}</td>
                                        <td>{{$ServiceOverview->service_overview_details}}</td>
                                        <td>{{$ServiceOverview->get_category->category_name}}</td>
                                        <td> <img style="height: 70px; width:auto;" src="{{url($ServiceOverview->image)}}" alt="image"></td>
                                        <td>
                                            <div class="row">
                                                <div>
                                                    <a href="{{route('ServiceOverviewDetails.edit' , $ServiceOverview->id)}}" style="color: white;" class="btn btn-primary m-2"> Edit </a>
                                                </div>
                                                <div>
                                                    <form action="{{route('ServiceOverviewDetails.destroy', $ServiceOverview->id)}}" method="POST">
                                                        @csrf
                                                        @method('Delete')
